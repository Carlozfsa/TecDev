...
@section('content')
    ...
    <div class="panel panel-default table-responsive">
        <div class="panel-body">
            ...
            <table>
                <tr>
                    <td style="padding-left: 465%;padding-bottom: 1%;"><button type="button" class="btn btn-success" onclick="location.href='/papelera_alumnos';">Alumnos borrados</button></td>
                </tr>
            </table>
                    ...
                    @foreach($a as $al)
                        <tr>
                           <td>{{$al->numero_control}}</td>
                            ...
                            <td>{{$al->alumno->turno->turno}}</td>
                            <td>{{$al->alumno->sem}}</td>
                            <td>{{$al->is_active==1 ? 'Activo' : 'Inactivo'}}</td>

                            {!! Form::open(['method'=>'GET', 'action'=>['AlumnosController@create_edit_alumnos', $al->numero_control]]) !!}
                            <td>{!! Form::submit('Editar', ['class'=>'btn btn-info'])!!}</td>
                            {!! Form::close() !!}


                            {!! Form::open(['method'=>'POST', 'action'=>['AlumnosController@activar_alumno', $al->numero_control]]) !!}
                            <td> @if($al->is_active == 0) {!! Form::submit('Activar', ['class'=>'btn btn-info col-sm-12'])!!} @else {!! Form::submit('Desactivar', ['class'=>'btn btn-warning col-sm-12'])!!} @endif</td>
                            {!! Form::close() !!}

                            {!! Form::open(['method'=>'POST', 'action'=>['AlumnosController@softDelete_alumno', $al->numero_control]]) !!}
